...
public class CalendarController {
    ...
    private void buildCalendar(LocalDate monthDate) {
        ...
        monthYearLabel.setText(monthDate.getMonth().getDisplayName(TextStyle.FULL,
                new Locale("ru")).toUpperCase() + " " + monthDate.getYear());
    ...
    private void onTimeSelected(String time, String dateStr) {
        ...
        // сохраняем выбранную дату и время в хранилище CurrentAppointment
        CurrentAppointment.setSelectedDate(dateStr);
        CurrentAppointment.setSelectedTime(time);

        try {

            // загружаем fxml-файл формы записи
            Parent root = FXMLLoader.load(getClass().getResource("/client/booking-form-view.fxml"));

            // получаем текущее окно (stage) из сцены, в которой находится контейнер timeSlotsContainer
            Stage stage = (Stage) timeSlotsContainer.getScene().getWindow();
            stage.setScene(new Scene(root)); // устанавливаем новую сцену в текущем окне
